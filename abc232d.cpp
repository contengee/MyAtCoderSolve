...
int main(){
    int H, W; cin >> H >> W;
    vector<string> C(H); rep(i, H) cin >> C[i]; // C[i][j] := (i, j)のマスの状態
    vector dp(H, vector(W, -INF)); dp[0][0] = 1; // dp[i][j] := (i, j)に到達するまでに通ったマスの数 (1-indexed)
    rep(i, H) rep(j, W) { 
        if(C[i][j] == '#') continue; // 壁は通れない
        if(i<H-1 and C[i+1][j]=='.') chmax(dp[i+1][j], dp[i][j]+1); // 下に移動
        if(j<W-1 and C[i][j+1]=='.') chmax(dp[i][j+1], dp[i][j]+1); // 右に移動
    }
    int ans = 0;
    rep(i, H) rep(j, W) chmax(ans, dp[i][j]); // 最大値を取る
    cout << ans << ln;
}
