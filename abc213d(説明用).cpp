...
int main(){
    ...
    vector<int> ans;
    auto dfs = [&](auto dfs, int v, int p) -> void{ // ラムダ式 vertex、parent
        ans.push_back(v); // 行きがけ順

        for(auto nv : G[v]){ // 子ノード
            if(nv == p) continue; // 親ノードはスルー
            dfs(dfs, nv, v); // 子ノードを順に見ていく
            ans.push_back(v); // 帰りがけ順
        }
    };
    dfs(dfs, 0, -1); // 0を根として探索 頂点0の親は-1
    for(auto v : ans){
        cout << v+1 << " ";
    }
    cout << endl;
